...
3. If the workspace has the Auditor enabled (**Settings → Agents**), the Auditor is queued: it reads the real diff against the matched topics' recorded claims.
...
- **Auditor off → no comment.** With the toggle off the PR is still observed and recorded, just not commented.
