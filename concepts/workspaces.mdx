...
A workspace is the top-level container for everything in Driftless: topics, their governance lifecycle, members, repos, and integrations all live inside one. Every topic belongs to **exactly one** workspace, and access is always scoped to it.

A user can belong to **many** workspaces and switch between them — in the dashboard, the CLI, or an MCP client — without signing in again.
...
| **admin** | Approve/vouch context, invite members, manage roles (not owner) |
| **member** | Propose context (agents propose; humans approve) |
...
1. **Membership is the truth.** A scoped credential can only reach a workspace while its human is *currently* a member. Removing someone from a workspace revokes their credentials there immediately — no reissue needed.
2. **No escalation.** Inside each workspace, a credential acts with that human's live role there, never a higher one.

## Invitations

Owners and admins invite teammates by email (Settings → Members, or the API). Driftless creates a single-use, 7-day invitation and emails a link. The invitee signs in and joins as a member — no external organization required.
...
Moving **resets the topic to a draft** in its new workspace — a vouch is workspace-specific and doesn't carry across — and drops workspace-local anchors (repo links, relations). It requires owner/admin in the source workspace and membership in the target.
