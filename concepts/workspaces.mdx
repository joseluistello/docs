...
Everything belongs to **exactly one** workspace, and access is always scoped to it. A user can belong to **many** workspaces and switch between them — in the dashboard, the CLI, or an MCP client — without signing in again.
...
| **member** | Write notes (agents write notes; humans add them to knowledge) |
...
Moving **resets the topic to a Note** in its new workspace — Knowledge status is workspace-specific and doesn't carry across — and drops workspace-local anchors (repo links, relations). It requires owner/admin in the source workspace and membership in the target.
