...
That note rides the card's `next` context bundle (the next loop tick sees it), and when the project closes, the **compact** synthesizes the card's attached notes into a draft Topic — sent to the Inbox for a human to merge into Knowledge. Work boundaries are where memory should attach: `next` is the moment to inject context, card-close is the moment to capture it.
