...
- **Note**: a draft or private observation. Only you see it; it's excluded from search; untouched notes auto-archive after ~14 days (recoverable). Think here, then propose it for review when it is ready.
...
The dashboard mirrors the three stages: **Knowledge** (the Knowledge graph, merged-in notes only), **Review queue** (the merge/reject queue plus re-review of drifted context), and **Notes** (your private workspace, with an "expiring soon" state). Notes are not a teammate-inbox or notification surface; use a Collection or the commercial Inbox for operational work.
