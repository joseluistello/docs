...
For every file in a scanned repository, Driftless checks whether any topic covers it — deterministically, with no LLM involved. The result is a **coverage map** that shows exactly what's documented and what's a knowledge gap.
