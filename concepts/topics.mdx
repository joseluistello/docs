...
A note becomes Knowledge once it's merged in: `draft → proposed → reviewed → archived`. `reviewed` is Knowledge; the read response carries `governance.authoritative`. Agents write notes, humans add them to knowledge. See [Governance](/concepts/governance).

| Status | Meaning |
|--------|---------|
| `draft` | Captured, not yet merged in — a hint (a Note), not truth |
| `proposed` | Up for review |
| `reviewed` | **Knowledge** — a human merged it in; the agent treats it as truth |
| `archived` | Retired |
