...
A topic lives in the **workspace** and is visible to all its members. The exception is a **private draft** (a draft with `is_private` set), visible only to its creator until proposed or un-marked. Need isolation between groups? use a separate workspace.
