...
A topic is **reviewed only if approved**: `draft → proposed → reviewed → archived`. `reviewed` is the reviewed state; the read response carries `governance.authoritative`. Agents propose, humans approve. See [Governance](/concepts/governance).

| Status | Meaning |
|--------|---------|
| `draft` | Captured, not yet blessed — a hint, not truth |
| `proposed` | Submitted for review |
| `reviewed` | **Reviewed** — a human approved it; the agent treats it as truth |
| `archived` | Retired |
