...
Only Knowledge (`reviewed`) topics auto-inject. A Note is a hint, not the team's truth — it never reaches the agent automatically. So **adding a note to knowledge is literally what makes your agents start coding with it.** The human gate and the hook are the same loop: review → merge → the hook delivers it → the agent gets it right.
