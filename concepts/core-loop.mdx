...
description: 'Scan → Observe → Contextualize → Deliver → Check'
---
...
The scanner extracts components from your TypeScript codebase. It reads the AST (via ts-morph) and identifies:
...
Pushes to tracked branches can mark topics as stale. PRs themselves don't trigger staleness — they trigger observation only.
...
Structural staleness goes beyond file matching — it detects deleted endpoint files, removed guards, and services that no longer exist in the codebase. Coverage reports show what's documented vs. what's not, with risk-based prioritization.
...
**CLI** (`@driftless-sh/cli`, npm): 24 commands — `init`, `sync`, `context` CRUD, `graph`, `install-skill`, `doctor`, `branches`, and more. Bundled with esbuild for fast startup. The primary interface agents and developers use to interact with Driftless.
...
**Agent Skill** (`SKILL.md`): Instructions for coding agents (Claude Code, Cursor). Teaches the `sync` → `load` → `edit` → `persist` workflow so agents automatically check and contribute context during coding sessions.
...
**Tech stack**: NestJS + TypeScript + Turborepo + Supabase + Clerk + Render + Vercel.
