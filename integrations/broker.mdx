...
- **Criterion is a reference, not a parse.** A criterion is a Knowledge topic slug attached to a Connection. The Broker never reads the topic's *content*; it resolves only status columns — the effective-context read (`GET …/context`, CLI `broker criterion <provider>`, MCP `action:'context'`) returns each slug with its trust (`knowledge`/`proposed`/`note`), staleness, and whether it still resolves, audited as a read receipt (`broker.context.read`). Attach validates the slug against a live topic (`UNKNOWN_TOPIC` otherwise) and is audited; detach never validates (a dead slug must always be removable). The agent reads the bodies with its own context tools before a risky write. `invoke` does not hard-require criterion; the non-executing `preview` does.
...
- [Guide: Use external context in operational work](/guides/external-context-with-broker) - the read-only and gated write paths.
