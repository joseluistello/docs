...
<Note>
Connecting a provider does **not** by itself make its data appear in context retrieve. External data is not Cerebro. It reaches retrieve only after an explicit index step and an explicit connector-source retrieve request. See the [Broker reference](/integrations/broker).
</Note>
...
- [Broker reference](/integrations/broker) - provider operations, gates, and read semantics.
