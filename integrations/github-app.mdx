...
The only comment Driftless posts is the **Auditor's**, and only when it has a finding. When it does, the affected topics' recorded context (gotchas, decisions, invariants, stale flags) rides along with it. A clean PR, or a workspace with the Auditor toggled off, gets no comment. See [PR Comments](/concepts/pr-bot).
