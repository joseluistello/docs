...
**Recorded context:** Each matched topic gets a block showing its `what` summary, relevant decisions, gotchas, and invariants — plus whether it's authoritative (`reviewed`) or just a hint (`draft`), and whether it's stale. The reviewer reviews *with* the team's context instead of without it.
...
auth-boundaries  ✓ authoritative   applies to: src/auth/session.ts, src/auth/tenant.ts
