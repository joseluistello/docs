...
The same eleven operations are reachable from four surfaces that all call the same services in-process: this HTTP API, the MCP tool `driftless_market_data`, the CLI's `driftless market` command, and the agent tool belt used by [chat and Investigations](#chat-and-due-diligence-screening). None of them run a model or SQL behind the scenes — the caller plans the sequence and synthesizes the answer.
...
| `get_supplier_batch` | `POST suppliers/batch` | no (bounded batch) | Use 1–50 opaque `record_ref` values returned by supplier search |
...
`get_supplier_batch` opens up to 50 already-selected observations in one call, preserves first-seen order, deduplicates repeated identities, and applies the same contact and licensing boundary as `get_supplier`. Invalid, unavailable, and unlicensed references intentionally share one `invalid_or_unavailable` status.
...
- **`page`** is `null` whenever the operation doesn't paginate (`get_supplier`, `get_supplier_batch`, `get_opportunity`, `market_capabilities`, `count_suppliers`, `compare_segments`).
...
The canonical API above is the **full** projection: `source_slug`,
`source_record_id`, `publisher`, `source_url`, retrieval timestamps and
`licensedForDisplay` all travel with it. An integrator, the dashboard, support
and the audit record read that.
...
Market-data routes require the same authentication as the rest of the API (API key or OAuth bearer token) — there is no public route. Over OAuth, they map to a dedicated `market_data:read` scope, OR-listed with `context:read` for back-compat:
...
| `context:read` | Still satisfies every market-data route transitionally: today's default OAuth issuance (the ChatGPT plugin, the MCP connector, CLI OAuth) does not yet grant `market_data:read`, so retiring `context:read` from this OR-list is a later, deliberate step, not automatic. |

## MCP: typed market-data tools

MCP exposes fourteen typed `driftless_market_*` tools, one per semantic operation, with external schemas for required fields, enums, arrays and limits. `driftless_market_data` remains a **deprecated compatibility shim** for existing `action`-based clients; new clients should use the typed tools. Reconnect a client after a schema update because connectors may cache tool definitions. See [MCP reference](/mcp/overview) for connecting a client.
...
<Note>
MCP clients (claude.ai, ChatGPT) **cache tool schemas per connector session**. When `driftless_market_data`'s parameters change, reconnect the connector (or start a fresh session) to see the new fields — see [MCP & OAuth](/mcp/overview) for the general caching note.
</Note>
