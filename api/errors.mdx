...
```json
{
  ...
  "message": "Creating a private project requires a human identity. Use a human-owned key or the dashboard, or create it as public.",
  "request_id": "a1b2c3d4",
  "retryable": false,
  "endpoint": "POST /api/v1/workspaces/acme/projects"
}
```
...
1. **Validation** rejects a bad enum (`status`, `kind`, `visibility`) with a field-level `400` *before* it touches the database.
...
| `MISSING_HUMAN_IDENTITY` | 403 | An ownerless agent key tried a human-gated action (create a private project, approve a topic). Re-issue the key from the dashboard so it carries your identity, or do it as a human. |
| `NOT_A_MEMBER` | 403 | You are not a member of this private project. |
| `NOT_FOUND` | 404 | The resource doesn't exist — or is in a private project you can't see (we return 404, never 403, to avoid revealing it). |
...
| `PROTECTED_RESOURCE` | 403 | The default "General" project can't be modified or deleted. |
