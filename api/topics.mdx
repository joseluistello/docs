...
A topic is authoritative only if approved. See [Governance](/concepts/governance). `approve` / `reject` and proposal `merge` / `reject` require a human identity.

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/workspaces/:slug/topics/:topic/propose` | `draft → proposed` |
| `POST` | `/workspaces/:slug/topics/:topic/approve` | `→ reviewed` (authoritative) |
