...
The CLI, package names, and API keys keep the `driftless` namespace during the compatibility window. Public MCP tools use neutral capability names; both `brein_*` and `driftless_*` calls remain hidden compatibility aliases. Existing integrations continue to work, and new links use the Brein domains shown in these docs.
