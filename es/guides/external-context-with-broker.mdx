...
description: "Tira evidencia de una herramienta conectada hacia un Project o Collection mediante el Broker, sin confundirla con Knowledge ni con el retrieve normal. Read-only hoy; el camino de escritura se describe pero está gated."
---

A veces la evidencia que necesita un trabajo vive en una herramienta que Driftless no posee: una página de Notion, un record en otro sistema. El [Broker](/es/integrations/broker) es cómo un Project o Collection alcanza esa evidencia mediante una [Connection](/es/integrations/overview) gobernada y auditada, la lee y la aterriza en la primitiva correcta, sin nunca tratarla como Knowledge ni como un retrieve normal de contexto.
...
<Warning>
**El Broker está gated.** Está apagado por defecto en producción (`DRIFTLESS_BROKER_ENABLED`), la lane externa OAuth/MCP es un interruptor aparte (`DRIFTLESS_BROKER_ROLLOUT`), y los callers externos además necesitan un **grant** por principal. El único conector de producción es **Notion**, **Beta y read-only**. **Ninguna operación de escritura es invocable en producción** (el registry de política de operaciones va vacío). Y el **GitHub App no es una GitHub Broker Connection**: observa pull requests y pushes para drift y el Auditor, y no expone `invoke`.
</Warning>

## Resultado

Un hecho externo específico tirado hacia el objeto que lo necesita: el contenido de una página de Notion leído en vivo y escrito en un Record, una Card o una Nota, con su procedencia preservada y su estatus como evidencia externa (no Knowledge) mantenido explícito.

## Cuándo usarlo

- Un Record o una Card tiene una brecha de información concreta, y la respuesta vive en una herramienta externa conectada.
...
| **Record / Card / Nota** | Donde aterriza la evidencia externa. |
...
```bash
driftless project card next <project-id>       # una Card lleva su context bundle
...
Escribe lo que encontraste en estado operacional, no en Knowledge. Un field del Record, una nota en la Card o una Nota independiente, citada a su fuente.
...
- **Un valor actual** va en el Record o la Card, citado a la fuente.
...
- **No asumas que el GitHub App puede `invoke`.** Observa; una GitHub Broker Connection para escrituras no está shipped.
...
| GitHub App (observación de PR y push) | Available (superficie aparte) |
