...
brief por defecto** aplica a Collections: `driftless_collection` `list` es active-only
(opta a archived con `include_archived:true`), y `get` es brief salvo que pases
`view:'full'`. Ver [Payload views](#payload-views-keep-reads-fast)
más abajo.
...
| Collections y operaciones | Collections, records y entities | [MCP: Collections y operaciones](/es/mcp/work) |
...
| Uso comercial | `driftless_usage` consulta el plan, solicitudes, créditos, renovación, investigación promocional y la acción para hablar con ventas. La consulta no consume saldo. | [Resumen de la API](/es/api/overview#uso-comercial) |
| Market data | Trece tools tipadas `driftless_market_*` para análisis más `driftless_contact_quote` y `driftless_contact_unlock`. No hay router genérico, detalle masivo de proveedores, selectores físicos ni coordenadas de contacto en el análisis. Reconecta los clientes después de cambios de schema porque pueden guardar definiciones de tools en caché. | [Market Data](/es/api/market-data) |

La superficie cubre el **contexto** (topics, governance, el grafo), la **superficie de trabajo** (tags y areas), el **sustrato operacional** (collections, records, entities), y la **ejecución del broker** (operar providers conectados). Nunca cubre la administración del workspace, el registro de repos, la gestión de keys, el **setup de integración** (conectar un provider), escribir/desplegar scripts de integración, ni disparar corridas de agentes a mano. Los topics viven en el workspace y son visibles para sus miembros; los drafts privados de otra persona quedan ocultos. Las escrituras en la superficie de trabajo requieren el scope `work:write`; mergear a Knowledge requiere autoridad de owner/admin, y el cliente MCP lo corre solo cuando el owner/admin que autoriza lo pide de forma explícita.
...
- **`full`**: todo, incluyendo el pesado cuerpo `content`. **Nunca es default en otro
  ...
  `view: "full"` aplica a `driftless_collection`
...
trabajo lo refleja — `driftless_collection` `action:'get'` en brief,
`view:'full'` para la única collection cuya config completa necesitas.
...
`driftless_<provider>_records` y `driftless_<provider>_document_content` (por ejemplo,
`driftless_notion_document_content`). Aparecen solo cuando la síntesis de tools del broker está
...
Los agentes remotos no tienen un checkout local, así que las respuestas de escritura llevan `anchor_validation`, conteos de coincidencias por patrón contra la **rama default** del repo: `ok`, `overbroad` (>100 archivos), o `zero`. Las advertencias nunca bloquean la escritura (un glob que ancla código de una rama sin mergear muestra 0 de forma legítima); léelas en el resultado de la herramienta y aprieta los globs marcados. Donde el deployment no tiene índice de archivos del repo vinculado, el campo llega como `{ skipped }` con la razón en vez de conteos — un «no se midió» honesto, nunca un pase silencioso.

<Note>
Los clientes MCP (claude.ai, ChatGPT) **cachean los schemas de herramientas por sesión de conector**. Después de que Driftless publica un cambio de schema, reconecta el conector (o arranca una sesión nueva) para tomarlo. Si no, herramientas retiradas pueden seguir apareciendo. Las herramientas de la superficie de trabajo (`tags`, `areas`, `driftless_collection`, `driftless_collection_record`) son **basadas en `action`**: pasa `action` para elegir la operación.
</Note>
...
La aplicación publicada en ChatGPT se llama **Brein**. Se conecta al mismo espacio
de trabajo autenticado de Driftless y usa los mismos permisos y contratos de
herramientas MCP; el nombre no crea una segunda cuenta ni otra frontera de datos.
...
4. El cliente recibe un bearer token y lo usa para las llamadas MCP dentro del espacio de trabajo autorizado
...
| `work:write` | Escribe la superficie de trabajo: collections, records, entities, comentarios, tags, areas, más las acciones de merge y share |
...
| `commercial:read` | Lee una cotización exacta de Contact Path para registros CRM seleccionados; nunca devuelve coordenadas de contacto. |
