...
| `driftless_broker` | **Opera** un provider ya conectado: una herramienta, `action`: `connections`, `operations`, `invoke`, `records`, `events`, `criterion`. Las credenciales se resuelven del lado del servidor y cada llamada queda auditada. El broker nunca **conecta/desconecta** un provider (eso es setup de integración, se hace en el dashboard o vía el flujo connect del CLI) y nunca escribe scripts |
...
| `driftless_agent_stats` | Estadísticas de notas de agentes |
