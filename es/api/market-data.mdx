...
MCP expone trece tools tipadas `driftless_market_*` para análisis más `driftless_contact_quote` y `driftless_contact_unlock`. No existe un router genérico por `action`, detalle masivo de proveedores ni selectores de fuente física. Reconecta el cliente después de un cambio de schema porque los conectores pueden guardar definiciones en caché. Ver [Referencia MCP](/es/mcp/overview) para conectar un cliente.
