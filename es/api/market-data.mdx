...
`results` lleva una entrada por cada RFC pedido, **en el orden pedido**, incluyendo un RFC con cero marcas — esa es una respuesta válida, respaldada por coverage, para ese RFC solamente, nunca evidencia sobre otro RFC del mismo batch. Más de 50 RFCs se rechaza con `batch_too_large` en vez de truncarse en silencio.
...
Cada respuesta exitosa — search, get, aggregate, count, screen por igual — es un solo envelope, y cada key en él es **camelCase**, deliberadamente distinta del snake_case que usa el resto de la plataforma (Knowledge, Collections, Projects). Esto no es una inconsistencia por corregir: es el dialecto que ya habla el contrato de market-data en producción, el resultado de la tool de MCP, y el plugin de ChatGPT.
...
- **`page`** es `null` cuando la operación no pagina (`get_supplier`, `get_opportunity`, `market_capabilities`, `count_suppliers`).
...
## MCP: `driftless_market_data`

Una tool de MCP compacta, once valores semánticos de `action` (los mismos nombres de operación de la tabla de arriba). Ver [Referencia MCP](/es/mcp/overview) para conectar un cliente.
...
<Note>
Los clientes MCP (claude.ai, ChatGPT) **cachean los schemas de herramientas por sesión de conector**. Cuando cambian los parámetros de `driftless_market_data`, reconecta el conector (o arranca una sesión nueva) para ver los campos nuevos — ver [MCP y OAuth](/es/mcp/overview) para la nota general sobre este caching.
</Note>
...
1. Resolver identidad por RFC primero — de un resultado previo de `search_suppliers`/`search_awards` o dado directamente — y llamar `search_risks { rfc }` / `search_permits { holder_rfc }` con él. La resolución por nombre aproximado (`entity_name` / `holder_name`) es el fallback, con la advertencia del match declarada.
