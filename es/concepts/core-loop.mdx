...
description: "Los dos tipos de estado que un workspace conecta, y el loop que convierte una señal en Knowledge revisado."
---
...
El estado de una empresa suele separar el contexto durable de los registros operacionales. Driftless mantiene ambos en un solo workspace, para que se alimenten entre sí en lugar de separarse.
...
Ninguno de los dos es el producto por sí solo. El producto es el loop entre ellos:
...
- Un mensaje de un cliente puede actualizar un Record en una Collection, entregarle al agente los criterios que el equipo registró para ese tipo de acción y dejar un aprendizaje revisado para la siguiente persona o agente.
...
Los dos tipos de estado son el producto. Todo lo demás es una forma de llegar a ellos, o un participante que actúa sobre ellos.
...
Guardar texto no basta, porque una afirmación vieja en la que un agente confía puede ser peor que ninguna. Cuando el código cambia en una rama rastreada, los Topics cuyos anchors se solapan con los archivos cambiados quedan marcados como **stale**, con una razón legible para humanos. El drift te llega como un badge cuando haces `context get` del área, así no tienes que ir a buscarlo. Los matches de pull requests se registran como actividad sin publicar comentarios de revisión. Así se mantiene honesto el Knowledge: sabe cuándo pudo dejar de ser verdad y pide que lo vuelvan a mirar.
