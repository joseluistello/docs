...
contract applies to the work surface: `driftless_project` / `driftless_collection`
`list` is active-only (opt into archived with `include_archived:true`), and `get`
...
| Market data | `driftless_market_data`: suppliers, opportunities, awards, risk marks, and permits — eleven semantic actions, no model or SQL behind them | [Market Data](/api/market-data) |
...
- **`full`**: everything, including the heavy `content` body. **Never a default
  ...
  The same `view: "full"` opt-in applies to `driftless_project` / `driftless_collection`
...
need, instead of pulling every full body up front. The work surface mirrors this —
`driftless_project` / `driftless_collection` `action:'get'` in brief, `view:'full'`
for the one project/collection whose full config you need.
