...
| `driftless_note_add` | Capture a private note (stays a draft; never submitted for review) |
...
| `driftless_project_card` | Project cards (units of work) — one tool, `action`: `list`, `get`, `add`, `set` (status/owner), `move`, `delete`. Needs `work:write` |
