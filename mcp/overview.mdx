...
| Tool | Description |
|------|-------------|
| `driftless_context_search` | Full-text search across topics — relevance-ranked, capped at 50. Accepts `tags` to scope the search to a category (e.g. `["billing"]`) |
| `driftless_context_list` | List topics, relevance-ranked (drifted-first, then most-recent). Returns the top 40 by default with `has_more` when more exist — narrow with `status`/`stale`/`tags`, or raise `limit`. Returns lightweight summaries; fetch a topic's full body with `driftless_context_get` |
