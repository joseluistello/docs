...
| Tool | Description |
|------|-------------|
| `driftless_context_retrieve` | **Start here when you have a task but no slug.** Ranked, drifted-first, bounded context for a `task` and/or `files`. Composes search + match-files + list. Bodies are `brief` by default; pass `view: "full"` or `driftless_context_get` one topic. Returns `{ shown, has_more, next_action, results[] }` with `match_type` / `why_matched` / `confidence` per hit |
...
| `driftless_context_get` | Retrieve a single topic by slug: the **full** body by default (you named the one you want); pass `view: "brief"` for a cheap second look |
| `driftless_context_get_for_files` | Match topics to a list of file paths, drifted ones first, bounded to the top 10 (raise `limit`, max 25). Bodies are **brief** by default (~76% lighter); pass `view: "full"` for full bodies |
...
| `driftless_collection` | Collections: one tool, `action`: `list`, `get`, `add`, `update`, `doctor`, `context` (resolve the criterion Knowledge to read before working a record), and **`retrieve`** (relevant records **plus** that criterion in one call). The operational substrate (CRM/tracker/content) |
| `driftless_collection_record` | Records (typed rows in a collection): one tool, `action`: `list`, `get`, `add`, `update`. A record's `status` is validated against its Collection's stages |
| `driftless_entity` | Entities (cross-collection identity & dedup): one tool, `action`: `list`, `get`, `upsert` (idempotent on `(kind, dedup_key)`) |
| `driftless_broker` | **Operate** an already-connected provider (flag-gated; see [Broker](/integrations/broker)): one tool, `action`: `connections`, `operations`, `capabilities`, `inspect`, `invoke`, `models`, `records`, `page-content`, `index-preview`, `index`, `events`, `attach-criterion`, `detach-criterion`. Credentials resolve server-side and every call is audited. Being listed is not being usable: an external caller is still gated by rollout and grants. The broker never **connects/disconnects** a provider (that's integration setup) and never authors scripts |
| `driftless_context_comment` | Comments: one tool, `action`: `add`, `list`, `resolve`. Points AT a topic / record / card; carries no governance, resolves into an edit |
...
| `driftless_context_merge` | Fold a duplicate topic into its survivor (mechanical; archives the source; prose never merged) |
| `driftless_context_propose` | Put a Note up for review (draft → proposed) |
| `driftless_context_approve` | Merge a Note into Knowledge (proposed → reviewed). **Owner/admin authority only**; the client runs it only when an owner/admin explicitly asks |
...
| `driftless_agent_stats` | Proposal outcome stats: how many agent proposals (the Auditor's Suggested edits) merged vs were rejected, i.e. the approval rate |
| `driftless_members` | List the workspace's members (member id, role, email): the valid `owner_clerk_id` values for assigning cards/records |
