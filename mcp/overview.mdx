...
| Tool | Description |
|------|-------------|
| `driftless_context_search` | Full-text search across topics — relevance-ranked, capped at 50 |
| `driftless_context_list` | List topics, relevance-ranked (drifted-first, then most-recent). Returns the top 40 by default with `has_more` when more exist — narrow with `status`/`stale`, or raise `limit`. Returns lightweight summaries; fetch a topic's full body with `driftless_context_get` |
...
<Note>
MCP clients (claude.ai, ChatGPT) **cache tool schemas per connector session**. After Driftless ships a schema change, reconnect the connector (or start a fresh session) to pick it up — otherwise new parameters like `rels` won't be available.
</Note>
