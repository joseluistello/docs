...
| `driftless_context_create` | Create a new topic (accepts `title`, and `project` to target a project) |
| `driftless_context_projects` | List the projects you can see (to pass as `project` on create) |
...
MCP is stateless, so there's no "active project" like the CLI. To put a topic in a specific project, the agent passes `project` (name or id) on `driftless_context_create` — discover the options with `driftless_context_projects`. Omit it and the topic lands in **General**. Identity-based access comes from the auth token: an OAuth-connected user (or an owned API key) sees and can write to their private projects; an ownerless key sees only public ones.
