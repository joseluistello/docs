...
| `driftless_broker` | **Operate** an already-connected provider: one tool, `action`: `connections`, `operations`, `invoke`, `records`, `events`, `criterion`. Credentials resolve server-side and every call is audited. The broker never **connects/disconnects** a provider (that's integration setup, done in the dashboard or via the CLI connect flow) and never authors scripts |
