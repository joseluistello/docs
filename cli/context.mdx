...
## Deprecated: context load

`context load` still works but is deprecated. Prefer `context get --files` for matching topics by path. The `load` command no longer returns scanner-derived fields (coverage tiers, graph facts, recommended actions).