...
|---|---|---|---|
| `context list` | `--stale`, `--suggested`, `--status`, `--repo`, `--docs`, `--auto`, `--manual`, `--limit <n>`, `--all`, `--json` | List topics with filters. Defaults to the **40 most relevant** (drifted first); prints `Showing N of M` — use `--all` / `--limit N` for more (`--json` is uncapped). `--suggested` = not-yet-authoritative (draft + proposed). | `driftless context list --suggested` |
...
| `context search` | query, `--json`, `--include-notes` | **Relevance-ranked** full-text search (top 50; name/title matches rank highest). Supports `"quoted phrases"`, `OR`, `-negation` — narrow with more terms rather than paging. Institutional Context + Proposals; Notes excluded unless `--include-notes`. | `driftless context search "payment"` |
