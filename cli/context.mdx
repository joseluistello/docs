...
| `tags rename <old> <new>` | — | Rename everywhere — rewrites every topic carrying the tag (owner/admin) | `driftless tags rename billing facturacion` |
| `tags rm <name>` | — | Delete from the registry **and** detach from every topic (owner/admin) | `driftless tags rm legacy` |
...
A topic becomes Knowledge only once it's merged in. Lifecycle: `draft → proposed → reviewed → archived`. Agents write notes; humans add them to knowledge. See [Governance](/concepts/governance).

| Command | Description | Example |
|---|---|---|
| `context propose <topic>` | Put a Note up for review | `driftless context propose payment-flow` |
| `context approve <topic>` | Add it to knowledge (human) | `driftless context approve payment-flow` |
...
| `context pr <topic> --merge <id>` | Merge a Suggested edit (human) | `driftless context pr payment-flow --merge <id>` |
| `context pr <topic> --reject <id>` | Close a Suggested edit without merging (human) | `driftless context pr payment-flow --reject <id>` |
...
The whole workspace is headless, not just topics. The same CLI drives the work surfaces — an agent runs the commands a human would:
