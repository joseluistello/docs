...
<Note>
The agent-first contract for [Collections](/concepts/collections) — intent `search`, bounded active-only reads, `--view full`, and `archive`/`restore`/`purge` as separate lifecycle operations — needs **`@driftless-sh/cli` ≥ 0.27.9**. Run `driftless --version` to verify; reinstall with the command above to upgrade.
</Note>

<Note>
The `usage` command and commercial-intelligence allowance contract require
...
| `usage` | `--json` | Show monthly commercial requests, credits, and renewal | `driftless usage` |
...
<AccordionGroup>
  <Accordion title="Environment Variables">
    ...
    | `DRIFTLESS_API_URL` | API server URL (default: `https://api.trybrein.com/api/v1`) |
  </Accordion>
</AccordionGroup>
