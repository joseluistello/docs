...
<Note>
The agent-first contract for [Projects](/concepts/projects) and [Collections](/concepts/collections) — intent `search`, bounded active-only reads, `--view full`, `close` with `context_outcome`, and `archive`/`restore`/`purge` as separate lifecycle operations — needs **`@driftless-sh/cli` ≥ 0.27.8**. Run `driftless --version` to verify; reinstall with the command above to upgrade.
</Note>
