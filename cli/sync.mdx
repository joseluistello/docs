...
`driftless sync` is the first command to run when you open a coding session.
...
|---|---|---|---|
| `sync` | `--json` | Show stale topics, recent PR activity, suggested topics, and tracked branches | `driftless sync` |
...
Team activity (2 PRs via GitHub App):
  [medium] #412 refactor auth middleware  → touched auth-boundaries

3 suggested topics from init — review and confirm:
...
| **Suggested topics** | `driftless init` suggestions | Confirm, enrich, merge, or delete them |
...
```bash
driftless context load --files "src/auth/session.ts,src/auth/token.ts"
```
...
| `suggested_pending` | Count of suggested topics waiting for review |
