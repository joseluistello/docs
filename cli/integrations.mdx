...
| `broker invoke <provider> <op> [--input <json>\|@file] [--idempotency-key <k>]` | Execute one operation inline; the key makes a write retry replay, never re-execute. |
| `broker criterion <provider> [--add <slug>\|--rm <slug>]` | Bare: the connection's EFFECTIVE context (criterion resolved to trust/staleness — read it before working the provider). With flags: attach or detach a topic slug. |
...
| `broker document <provider> <externalId>` | Read one document's bounded, live, citeable content from any content provider (`--max-bytes`). |
| `broker page <pageId>` | DEPRECATED Notion-only alias of `broker document notion <id>`. |
...
| `broker import <provider> --collection <id>` | Import mirror records into a Collection via its saved connector mapping (`--model`, `--modified-after`, `--max-pages`). |
