...
| Command | Description |
|---|---|
| `note add --content "..."` | Create a Cerebro document. `--content` accepts `@file`. |

`--allow-secrets` and `--json` are available. A document is born shared with the workspace; use the document visibility controls when you need a private or public visibility.
