...
`driftless init` scans the current repository, uploads a baseline to Driftless Cloud, and installs the agent skill so your agents can read and write context. See [Agent Setup](#agent-setup) for details on what the skill provides.
...
Running `driftless install-skill` again replaces the block contents. This means you can update the skill instructions without manual editing — just re-run the command and the block refreshes to the latest version.
