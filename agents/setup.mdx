...
2. **If no topic exists, create one** — `driftless context add "my-area" --kind reference --content "..."` (it lands as a draft).
3. **Persist what you learned** — `driftless context update <slug> --gotcha "..." --decision "..."`; for an already-approved topic, open a topic-PR instead: `driftless context pr <slug> --open --summary "..." --content @file`.
...
A topic is authoritative only once a human approves it (`reviewed`) — treat `reviewed` as truth, `draft`/`proposed` as a hint. See [Governance](/concepts/governance).
